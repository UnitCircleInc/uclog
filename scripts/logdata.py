...
def parse(filename):
  ...
  cmd = ['arm-zephyr-eabi-objdump', '-Wi', '-g', filename]
  # Support Nuitka builds which include arm-zephyr-eabi-objdump
  # binary in the assets folder, update cmd to point to it
  is_nuitka = "__compiled__" in globals()
  objdump_found = shutil.which(cmd[0])
  if is_nuitka or objdump_found is None:
    if os.name == 'nt':  # Windows
      f_objdump = os.path.join(os.path.dirname(__file__), 'assets/arm-zephyr-eabi-objdump.exe')
      cmd = [f_objdump, '-Wi', '-g', filename]
    elif os.name == 'posix':  # Linux (Ubuntu)
      f_objdump = os.path.join(os.path.dirname(__file__), 'assets/arm-zephyr-eabi-objdump')
      cmd = [f_objdump, '-Wi', '-g', filename]
    else:
      print("Unknown OS for arm-zephyr-eabi-objdump")
...
def process_subprogram(item, functions):
  if 'low_pc' in item and 'name' in item:
    ...
    if low_pc > 0:
      ...
      #print(item.tag, item['name'].short, item['low_pc'].value, item['high_pc'].value)
...
def load_logdata(fname):
  ...
  with open(fname, 'rb') as f:
    elf = ELFFile(f)
    section = elf.get_section_by_name('.logdata')
    if section == None:
      raise Exception(f".logdata section not found in {fname}")
    sdata = section.data()
...
def load_all(fname):
  with open(fname, 'rb') as f:
    elf = ELFFile(f)
    sdata = elf.get_section_by_name('.logdata_all').data()
...
def load_cbor_from_elf(filename):
  with open(filename, 'rb') as f:
    ...
    # Can't get app hash easily because signature isn't added to ELF
    app_hash = None
    if s:
      data = s.data()
      if len(data) > 0:
...
class LogData(object):
  def __init__(self, filename, verbose = False):
    ...
    if filename.endswith('.cbor') or filename.endswith('.logdata'):
      ...
      hash_str = os.path.basename(filename).split('.')[0]
      try:
        self.app_hash = bytes.fromhex(hash_str)
        self.autoreload = False
      except ValueError:
        self.app_hash = None
        self.autoreload = True
    else:
      data, self.app_hash = load_cbor_from_elf(filename)
      if data:
        self.enums, self.tdenums, self.variables, \
            self.functions, self.saddr, self.fmts = load_from_cbor(data)
      else:
    ...
    if verbose:
      print(f"Loaded target: {self.filename} target: {self.target_digit()}")

  def target_digit(self):
    return (self.saddr >> TARGET_DIGIT_SHIFT) & 0xf;

  def check_for_reload(self, device_app_hash=None):
    ts = os.stat(self.filename).st_mtime
    if ts != self.ts:
      old_target = self.target_digit()
      if device_app_hash is None and (self.filename.endswith('.cbor') or self.filename.endswith('.logdata')):
        with open(self.filename, 'rb') as f:
          data = f.read()
        self.enums, self.tdenums, self.variables, \
            self.functions, self.saddr, self.fmts = load_from_cbor(data)
      else:
        data, app_hash = load_cbor_from_elf(self.filename)
        if data:
          if device_app_hash and (app_hash != device_app_hash):
              print("App hash of modified file doesn't match device")
              return
      ...
      if self.target_digit() != old_target:
        print("Target changed from %d to %d" % (old_target, self.target_digit()))
  ...
  def decode(self, item):
    ...
    kind = addr & 3
    addr = addr & ~3
    (level, fname, line, clean, parser) = self.fmts.get(addr,
  ...
  def validate(self, device_app_hash):
    if self.autoreload:
      self.check_for_reload(device_app_hash)
    valid = (device_app_hash == self.app_hash)
    if valid:
      # Don't autoreload file anymore
      self.autoreload = False
    return valid
  ...
  def dump(self):
    ...
    for (low, hi) in sorted(self.functions.keys(), key=itemgetter(0)):
      print("[0x%08x, 0x%08x) - %s" % (low, low+hi, self.functions[(low, hi)]))
