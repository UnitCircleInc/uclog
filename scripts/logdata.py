...
re_header = re.compile(
    r"^\s*<(\d+)><([0-9a-fA-F]+)>: Abbrev Number: (\d+) \(DW_TAG_(.+)\)"
)
    ...
    if True:
        for lineno, line in enumerate(proc.stdout.readlines()):
            ...
            if m:
                ...
                if debug:
                    print("  addr: %s tag: %s detail: %s" % m.groups())
                continue
            ...
            if m:
                ...
                while stack[-1]["level"] >= int(m.group(1)):
                    if debug:
                        print("Poping level", stack[-1]["level"])
                    x = stack.pop()
                    stack[-1]["item"].add_child(x["item"])

                if stack[-1]["level"] + 1 != int(m.group(1)):
                    print("Skipping levels", lineno, line)
                    exit(-1)

                if debug:
                    print("Pushing a new level", int(m.group(1)))
                stack.append({"level": int(m.group(1)), "item": item})
                if debug:
                    print("level: %s addr: %s rev: %s tag: %s" % m.groups())
                continue
...
def lookup_var(variables, a):
    # Only consider valid if 0 <= offset < 0x4000
    d = [
        (variables[addr], a - addr)
        for idx, addr in enumerate(variables.keys())
        if a >= addr and a - addr < 0x4000
    ]
    if len(d) == 0:
        return None
    else:
...
re_fmt = re.compile(
    r"(((?<!%)|(?P<sym>{sym})|({enum:(?P<enum>[^}]*)}))%[#+\- 0]*[0-9]*\.?[0-9]*(?:lu[fFeEgGaA]|[l]*[diouxXcfFeEgGaApsb]|z[douxX]))"
)
...
def load_logdata(fname):
    ...
    with open(fname, "rb") as f:
        ...
        fmt = {m.start() + saddr: parse_prefix(m.group(1).decode()) for m in strings}
...
def extract_vals(frame, parser, logdata):
    ...
    for p in parser:
        ...
        vals.append(v)
    if len(frame) > 0:
        return (None, "Extra data in frame %s" % (hex2str(frame)))
...
class LogData(object):
    ...
    def decode(self, item):
        ...
        kind = addr & 3
        addr = addr & ~3
        (level, fname, line, clean, parser) = self.fmts.get(
            addr, (None, None, None, None, None)
        )
        ts = int((time.time() - self.start_time) * 1000 + 0.5) / 1000.0
        self.count += 1
